...
export class LandingView extends Module {
    ...
    private _makeCircles(stars: PIXI.Sprite[]) {
        const { CIRCLE_SIZE, CIRCLE_OFFSET } = LandingView;
    ...
    private _makeCircle(
        ...
        for (let i = 0; i < n; i++) {
            ...
            const tx = x + LandingView.CIRCLE_SIZE * Math.cos(rads);
            const ty = y + LandingView.CIRCLE_SIZE * Math.sin(rads);
    ...
    private _moveCircles(step: number = 0) {
        const { CIRCLE_OFFSET, MAX_CIRCLE_ANIMATION_STEP } = LandingView;
